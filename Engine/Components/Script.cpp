...
{
	auto script = easepix::script::registry().find(tag);
	assert(script != easepix::script::registry().end() && script->first == tag);
...
get_script_names()
{
	const u32 size{ (u32)easepix::script::script_names().size() };
	if (!size) return nullptr;
	CComSafeArray<BSTR> names(size);
	for (u32 i{ 0 }; i < size; ++i)
	{
		names.SetAt(i, A2BSTR_EX(easepix::script::script_names()[i].c_str()), false);
	}
	return names.Detach();
}
#endif